...
(
    id         bigint auto_increment comment 'Primary Key ID'
        primary key,
    userID     bigint                              not null comment 'Collector User ID',
    goodsID    bigint                              not null comment 'Collected Product ID',
    createTime timestamp default CURRENT_TIMESTAMP not null comment 'Time of Collection',
    constraint uk_user_goods
        unique (userID, goodsID) comment 'Prevent duplicate collections by the same user'
)
    comment 'Product Wishlist/Collection Table' engine = InnoDB
                                                collate = utf8mb4_unicode_ci;
